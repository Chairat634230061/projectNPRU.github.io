...
if (isset($_POST['update'])) {
    ...
    if ($sql) {
        ...
        if ($user_status == 'อนุมัติ') {
            ...
            $insertstmt = $conn->prepare("INSERT INTO successful (name_activity, studentID, collect_hours, img, user_status, firstname, lastname, id) SELECT name_activity, studentID, collect_hours, img, user_status, firstname, lastname, id FROM join_activity WHERE id = :id");
            ...
            $insertstmt = $conn->prepare("INSERT INTO unsuccessful (name_activity, studentID, collect_hours, img, user_status, firstname, lastname, id) SELECT name_activity, studentID, collect_hours, img, user_status, firstname, lastname, id FROM join_activity WHERE id = :id");
            $insertstmt->bindParam(':id', $id);
            $insertstmt->execute();
